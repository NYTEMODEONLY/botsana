...
@bot.tree.command(name="reset-database", description="Reset database tables (ADMIN ONLY - DANGER!)")
@discord.app_commands.checks.has_permissions(administrator=True)
async def reset_database_command(interaction: discord.Interaction):
    """Reset and recreate all database tables. WARNING: This will delete all data!"""
    await interaction.response.defer()

    # Double confirmation
    embed = discord.Embed(
        title="⚠️ DANGER: Database Reset",
        description="This will **DELETE ALL DATA** in the database and recreate all tables.\n\nThis action cannot be undone!",
        color=discord.Color.red()
    )

    embed.add_field(
        name="What will be lost:",
        value="• Guild configurations\n• Error logs\n• Audit settings\n• All stored data",
        inline=False
    )

    embed.add_field(
        name="What will be fixed:",
        value="• Database schema issues\n• BIGINT field problems\n• Table structure problems",
        inline=False
    )

    embed.add_field(
        name="⚠️ Confirmation Required",
        value="Reply with `YES DELETE ALL DATA` to confirm this destructive action.",
        inline=False
    )
    ...
    try:
        ...
        if msg.content == "YES DELETE ALL DATA":
            ...
            try:
                ...
                embed = discord.Embed(
                    ...
                    description="All tables have been dropped and recreated.",
                    color=discord.Color.green()
                )
                ...
                await error_logger.log_system_event(
                    "database_reset",
                    f"Database reset initiated by {interaction.user}",
            ...
            await interaction.followup.send("❌ Database reset cancelled.")
    except asyncio.TimeoutError:
        await interaction.followup.send("❌ Database reset timed out - cancelled.")

@reset_database_command.error
async def reset_database_error(interaction: discord.Interaction, error):
    """Handle reset database command errors."""
    if isinstance(error, discord.app_commands.errors.MissingPermissions):
        embed = discord.Embed(
            title="❌ Administrator Required",
            description="You need Administrator permissions to reset the database.",
            color=discord.Color.red()
        )
        if not interaction.response.is_done():
            await interaction.response.send_message(embed=embed)
        ...
        logger.error(f"Reset database error: {error}")
